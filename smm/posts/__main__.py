...
def generate_title_for_post(client, post_filename):
    with open(post_filename, "r", encoding="utf-8") as file:
        post = file.read()
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": system},
            {"role": "user", "content": f"Створи текст поверх на картинці для посту {post}. Коротко та змістовно. Без кавичок, без emoji. 5-6 слів. Лише перше слово з великої літери. Креативний та цікавий заголовок, який залучить увагу."}
        ]
    )
...
def generate_text_under_post(client, post_filename):
    with open(post_filename, "r", encoding="utf-8") as file:
        post = file.read()
    # generate prompt for the title using the post
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": system},
            {"role": "user", "content": f"Створи підпис посту в Instagram, для тексту який розміщено на слайдах самого поста (на картинках) : {post}. Коротко та змістовно. Можна використати emoji. 5-6 слів. Лише перше слово з великої літери. Креативний та цікавий підпис, який залучить увагу. Збережи хештеги з посту."}
        ]
    )
    title = response.choices[0].message.content
...
def add_logo_and_border(main_image_path, logo_image_path, output_image_path, border_size=20, border_color=(0, 0, 0), font_path="Mariupol-Medium.otf", title_text="Nostalgia", title_size=50, title_color=(255, 255, 255), title_position=(20, 20),
                        arrow=False):
    # Load the main image
    ...
    margin = title_position[0]
    offset = title_position[1]
    for line in textwrap.wrap(title_text, width=20):
        draw.text((margin+5, offset), line, fill=(0, 0, 0), font=font)
        draw.text((margin-5, offset), line, fill=(0, 0, 0), font=font)
        draw.text((margin, offset+5), line, fill=(0, 0, 0), font=font)
        draw.text((margin, offset-5), line, fill=(0, 0, 0), font=font)
        draw.text((margin, offset), line, fill=title_color, font=font)
        offset += title_size  #font.getsize(line)[1]
...
if __name__ == "__main__":
    ...
    # filename = 'posts/FAQ/_FAQ____a80306.txt'

    generate_title_for_post(client, filename)
    generate_text_under_post(client, filename)
    ...
    add_logo_and_border(
        ...
        title_text=title, title_color=(255, 255, 255), title_position=(100, 200), title_size=90,
        arrow=True
    )
